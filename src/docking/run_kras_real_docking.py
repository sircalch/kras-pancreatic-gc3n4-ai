...
def prepare_receptor_7rpz(pdb_file, out_pdbqt):
    ligand_coords = []
    cleaned_lines = []
    
    with open(pdb_file, 'r', encoding='utf-8') as f:
        for line in f:
            if line.startswith("HETATM") and not line[17:20].strip() in ["HOH", "EDO", "DMS", "SO4", "PEG", "MG"]:
                x = float(line[30:38])
                y = float(line[38:46])
                z = float(line[46:54])
    ...
    if ligand_coords:
        ...
        center = np.array([18.5, 12.0, 5.0])
        
    print(f"KRAS-G12D Switch II Allosteric Pocket Center: X={center[0]:.3f}, Y={center[1]:.3f}, Z={center[2]:.3f}", flush=True)
...
def run_real_vina_docking():
    ...
    pdb_path = os.path.join(base_dir, "data", "raw", "7RPZ.pdb")
    receptor_pdbqt = os.path.join(base_dir, "data", "raw", "7RPZ_receptor.pdbqt")
    ...
    drugs_csv = os.path.join(base_dir, "data", "raw", "kras_drug_library.csv")
    
    os.makedirs(lig_dir, exist_ok=True)
    os.makedirs(poses_dir, exist_ok=True)
    
    center = prepare_receptor_7rpz(pdb_path, receptor_pdbqt)
    df_drugs = pd.read_csv(drugs_csv)
    
    print("\n=======================================================")
    print(f"  Starting Parallel Real AutoDock Vina Execution on {len(df_drugs)} KRAS Drugs")
    print("=======================================================")
