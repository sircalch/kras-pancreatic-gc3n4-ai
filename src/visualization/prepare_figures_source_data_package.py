...
- `oof_observed_vs_predicted_qspr.csv` -> Out-of-fold observed (GFN2-xTB QM) vs predicted (Ridge QSPR) Delta_E_ads for all 33 compounds, with residuals, standardized residuals, and hat-matrix leverages (hi).
  * Metrics (leak-free nested 5x5 CV): Q2_CV = +0.584, RMSE = 5.11 kcal/mol, MAE = 4.33 kcal/mol; per-fold Q2 0.05-0.78.
- `yscrambling_1000_permutations.csv` -> 1,000 Y-scrambling permutation iterations (mean Q2_scr = -0.12, empirical p-value = 0.0010).
